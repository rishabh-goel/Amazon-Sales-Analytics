...
def create_region_dim(all_sales_df, session) -> None:
    ...
    region_dim_df = region_dim_df.selectExpr("sales_dwh.consumption.region_dim_seq.nextval as region_id_pk", "Country",
                                             "Region", "isActive")
    ...
    region_dim_df = region_dim_df.join(existing_region_dim_df,
...
def create_product_dim(all_sales_df, session) -> None:
    ...
    existing_product_dim_df = session.sql(
        "select mobile_key, Brand, Model, Color, Memory from sales_dwh.consumption.product_dim")
    existing_product_dim_df.count()

    product_dim_df = product_dim_df.join(existing_product_dim_df, ["mobile_key", "Brand", "Model", "Color", "Memory"],
                                         join_type='leftanti')

    product_dim_df.show(5)

    product_dim_df = product_dim_df.selectExpr("sales_dwh.consumption.product_dim_seq.nextval as product_id_pk",
...
def create_promocode_dim(all_sales_df, session) -> None:
    promo_code_dim_df = all_sales_df.with_column("promotion_code", expr(
        "case when promotion_code is null then 'NA' else promotion_code end"))
    ...
    existing_promo_code_dim_df = session.sql(
        "select promotion_code, country, region from sales_dwh.consumption.promo_code_dim")

    promo_code_dim_df = promo_code_dim_df.join(existing_promo_code_dim_df, ["promotion_code", "country", "region"],
                                               join_type='leftanti')
...
def create_payment_dim(all_sales_df, session) -> None:
    payment_dim_df = all_sales_df.groupBy(col("COUNTRY"), col("REGION"), col("payment_method"),
                                          col("payment_provider")).count()
    ...
    existing_payment_dim_df = session.sql(
        "select payment_method,payment_provider,country, region from sales_dwh.consumption.payment_dim")
...
def create_date_dim(all_sales_df, session) -> None:
    ...
    date_dim_tuples = [tuple(row) for row in date_dim.to_numpy()]

    # Create Snowpark DataFrame from the list of tuples
    date_dim_df = session.create_dataframe(date_dim_tuples,
                                           ['order_dt', 'Year', 'DayCounter', 'Month', 'Quarter', 'Day', 'DayOfWeek',
                                            'DayName', 'DayOfMonth', 'Weekday'])

    existing_date_dim_df = session.sql('select order_dt from sales_dwh.consumption.date_dim ')
    date_dim_df = date_dim_df.join(existing_date_dim_df, existing_date_dim_df['order_dt'] == date_dim_df['order_dt'],
                                   join_type='leftanti')
...
def main():
    ...
    all_sales_df = all_sales_df.with_column("promotion_code",
    ...
    all_sales_df = all_sales_df.join(payment_dim_df, ["payment_method", "payment_provider", "country", "region"],
                                     join_type='inner')
